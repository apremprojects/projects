...
void MagnetometerWidget::paintGL() {
    ...
    }
    {
        ...
        while (!queue->empty()) {
            ...
            queue->pop_front();
            ring_buffer->load(ring_index, 1, &data.mag);
...
void MagnetometerWidget::setMutexAndQueue(QMutex& m, QQueue<MagnetometerData>& q) {
  mutex = &m;
  queue = &q;
}
...
void MagnetometerWidget::shiftCenter(int status) {
    ...
    qDebug() << "updating checkbox -> " << shift_center;
}
